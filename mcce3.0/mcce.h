...
typedef struct {
    ...
    float gaussian_sigma;     // The value of the variance of Gaussian distributio. This parameter affects how Gaussian function generates the dielectric distribution. From our benchmark against experimental measured pKaâ€™s on two datasets, the optimal value is 0.7 for surface residues; and 0.95 for buried residues.
    float gaussian_srfcut;    // The cutoff for the moleculeâˆ’water interface GAUSSIAN_SRFCUT
    ...
    int   display_delphi_io_final;  // This flag to display what is sent/received to delphi (Jan. 2016 by Salah)
...
int premcce();
int rotamers();
//int rotamers_GA(); // Commented out by Salah on Apr 2016
int energies();
int energies2();
int monte();
int monte2();
