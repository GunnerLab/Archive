...
int main(int argc, char *argv[])
{
    ...
    char tempID[256];
    int int_dummy;
    float temp_ele,temp_vdw, temp_crt, temp_ori, temp_pwBound, temp_kfactor;
    char  temp_mark[4];
    FILE *fp;
    int verbose;
    char **arguments;
        ...
        for (i=0; i<ematrix.n; i++) {
            if (!strcmp(ematrix.conf[i].uniqID, param)) {
                ...
                for (j=0; j<ematrix.n; j++) {
                    counter++;
                    printf("%05d %s %8.3f%8.3f%8.3f%8.3f%8.3f %s\n", counter, ematrix.conf[j].uniqID, ematrix.pw[i][j].ele, ematrix.pw[i][j].vdw, ematrix.pw[i][j].crt, ematrix.pw[i][j].ori,ematrix.pw[i][j].pwBound, ematrix.pw[i][j].mark);
                }
                match = 't';
            }
        }
        ...
        for (i=0; i<conflist.n_conf; i++) {
            ...
            if (!(fp=fopen(fname, "r"))) { /* not exist, assume no interactions  */
                ...
                while(fgets(sbuff, sizeof(sbuff), fp)) {
                    sscanf(sbuff, "%d %s %f %f %f %f %f %f %s", &int_dummy, tempID, &temp_ele, &temp_vdw, &temp_crt, &temp_ori, &temp_pwBound, &temp_kfactor, temp_mark);
                    ...
                    if (counter < ematrix.n) { /* a match was found */
						...
						ematrix.pw[i][counter].pwBound = temp_pwBound;
						ematrix.pw[i][counter].kfactor = temp_kfactor;
