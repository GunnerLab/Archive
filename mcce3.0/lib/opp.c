...
int main(int argc, char *argv[])
{
    ...
    char tempID[256];
    int int_dummy;
    float temp_ele,temp_vdw, temp_crt, temp_ori, temp_pwBound; // Salah
    char  temp_mark[4];
    FILE *fp;
    int verbose;
    char **arguments;
        ...
        for (i=0; i<conflist.n_conf; i++) {
            ...
            if (!(fp=fopen(fname, "r"))) { /* not exist, assume no interactions  */
                ...
                while(fgets(sbuff, sizeof(sbuff), fp)) {
                    sscanf(sbuff, "%d %s %f %f %f %f %f %s", &int_dummy, tempID, &temp_ele, &temp_vdw, &temp_crt, &temp_ori, &temp_pwBound, temp_mark);
                    ...
                    if (counter < ematrix.n) { // a match was found
						...
						ematrix.pw[i][counter].pwBound = temp_pwBound; // Salah added this on (Fed. 2016) 
