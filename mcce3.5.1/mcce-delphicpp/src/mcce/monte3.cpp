...
float score(float v[]);
void monte3_dhill(float **p, float *y, int ndim, float ftol, float (*funk)(float []), int *nfunk);
...
time_t   time_test=0, time_t1, time_t2;
...
int   monte3_verify_flag();
int   monte3_load_pairwise();
int   load_pairwise_fround3();
int   monte3_load_pairwise_vdw();
void  monte3_group_confs();
float monte3_get_base();
float monte3_get_E();
void  monte3_mk_neighbors();
void monte3_MC(int n);
int monte3_reduce_conflist();
int fitit();
int monte3_enumerate(int i_ph_eh);
int monte3_load_conflist();
int monte3_group_conftype();
int monte3_cmp_conftype(CONFTYPE t1, CONFTYPE t2);
CONFTYPE monte3_get_conftype(CONF conf);
void monte3_update_Sconvergence();
float monte3_s_stat();
float monte3_get_entropy(int iconf, int start, int end);
...
{  int i, j, k, counter, k_run;
    ...
    /* group conformers */
    monte3_group_conftype();  /* This ininitializes Sconverge and SconvergeBak */
    ...
    for (i=0; i<env.titr_steps; i++) {
        ...
        */
        ...
        if (env.monte_tsx) {
           while (S_max > 0.5) {
             E_base = monte3_get_base();
             if (monte3_enumerate(i) == -1) { /* Non-ANALYTICAL SOLOTION */
                counter = 0;
                for (k=0; k<n_free; k++) counter+=free_res[k].n;
                N_smp = env.monte_niter * counter;

                fprintf(fp, "Doing Entropy sampling cycle %d...\n", j+1); fflush(fp);
                if (env.monte_nstart * counter) monte3_MC(env.monte_nstart * counter);
                if (N_smp) monte3_MC(N_smp);
             }
             monte3_update_Sconvergence(); /* calculate entropy from occupancy */
             S_max = monte3_s_stat();
...
//            N_smp = env.monte_niter;
            ...
            for (j=0; j<env.monte_runs; j++) {
                ...
                if (N_smp) {
                    if (env.ms_out) MC_smp(N_smp);
                    else monte3_MC(N_smp);
                }
...
//            printf("time to update ms is %ld\n", time_test);
            time_test=0; 
...
{  int cycles, n_total, n_cycle;
    ...
    E_minimum = E_state = monte3_get_E();
    ...
    for (i=0; i<cycles; i++) {
        ...
        i*n_cycle, E_minimum+E_base, E_state+E_base, monte3_get_E()+E_base);
        */
...
{  float **p;
    ...
    monte3_dhill(p, y, 2, 0.0001, &score, &neval);
...
void monte3_dhill(float **p, float *y, int ndim, float ftol, float (*funk)(float []), int *nfunk)
{
    float monte3_dhtry(float **p, float *y, float *psum, int ndim, float (*funk)(float []), int ihi, float fac);
    ...
    *nfunk = 0;
    ...
    for (;;) {
        ...
        *nfunk += 2;
        ...
        ytry = monte3_dhtry(p,y,psum,ndim,funk,ihi,-1.0);
        
        if (ytry <= y[ilo]) ytry=monte3_dhtry(p,y,psum,ndim,funk,ihi,2.0);
        else if (ytry >= y[inhi]) {
            ysave = y[ihi];
            ytry=monte3_dhtry(p,y,psum,ndim,funk,ihi,0.5);
            if (ytry >= ysave) {
                for (i=0; i<mpts; i++) {
                    if (i !=ilo) {
                        for (j=0; j<ndim; j++)
...
int monte3_enumerate(int i_ph_eh)
{
    ...
    istate = 0;
    E_min = E_state = monte3_get_E();  /* get microstate energy */
...
{  int i;
   ...
   oldID = monte3_get_conftype(conflist.conf[0]);
   for (i=1; i<conflist.n_conf; i++) {
      newID = monte3_get_conftype(conflist.conf[i]);
      if (monte3_cmp_conftype(oldID, newID) == 0); /* same group of conformers */
      else {
         n++;
         oldID = newID;
      }
   ...
   if (!(Sconverge.conftype = (CONFTYPE *) malloc(n*sizeof(CONFTYPE)))) {
      printf("   Memory allocation error in \"monte3_group_conftype\"\n");
      return USERERR;
   }
   ...
   for (i=1; i<conflist.n_conf; i++) {
      newID = monte3_get_conftype(conflist.conf[i]);
      if (monte3_cmp_conftype(oldID, newID) != 0) {
         n++;
         oldID = newID;
...
{  int i, pstart, pend;
   ...
   for (i=0; i<Sconverge.n; i++) {
      while (pend < conflist.n_conf) {
         newID = monte3_get_conftype(conflist.conf[pend]);
         if (monte3_cmp_conftype(Sconverge.conftype[i], newID) == 0) {
            pend ++;
            continue; /* same group of conformers */
         }
         else break;
      }
      Sconverge.conftype[i].E_TS = monte3_get_entropy(i, pstart, pend);
      /* Have done that in monte3_get_entropy */
